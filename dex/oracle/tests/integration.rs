...
fn mock_app(owner: Option<Addr>, coins: Option<Vec<Coin>>) -> App {
    if let (Some(owner), Some(coins)) = (owner, coins) {
...
fn change_provide_liquidity(
    ...
    mint_some_token(router, owner.clone(), token1.clone(), user.clone(), amount1);
    mint_some_token(router, owner, token2.clone(), user.clone(), amount2);
    check_balance(router, user.clone(), token1.clone(), amount1);
    check_balance(router, user.clone(), token2.clone(), amount2);
    allowance_token(
        router,
        user.clone(),
        pair_contract.clone(),
