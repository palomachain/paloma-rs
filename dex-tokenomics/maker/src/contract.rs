...
/// ## Params
/// * **deps** is an object of type [`Deps`].
fn query_get_config(deps: Deps<PalomaQueryWrapper>) -> StdResult<ConfigResponse> {
...
///
/// * **env** is an object of type [`Env`].
fn query_bridges(deps: Deps<PalomaQueryWrapper>, _env: Env) -> StdResult<Vec<(String, String)>> {
    BRIDGES
        .range(deps.storage, None, None, Order::Ascending)
        .map(|bridge| bridge.map(|(bridge_name, asset_info)| (bridge_name, asset_info.to_string())))
        .collect()
}
