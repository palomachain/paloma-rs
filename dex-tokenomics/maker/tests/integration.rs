...
fn update_config() {
    ...
    assert_eq!(res.governance_contract, Some(new_governance));
    ...
    router
        .execute_contract(owner, maker_instance.clone(), &msg, &[])
        .unwrap();
...
fn collect_all() {
    ...
    let bridges = vec![
        (
            ...
            token_asset_info(bridge2_token_instance),
        ),
        (
    ...
    let expected_balances = vec![
        native_asset(uusd_asset, Uint128::new(100)),
        native_asset(uluna_asset, Uint128::new(110)),
    ...
    let collected_balances = vec![
        ...
        (astro_token_instance, 262u128),
        (usdc_token_instance, 0u128),
        (test_token_instance, 0u128),
    ];
...
fn collect_default_bridges() {
    ...
    let expected_balances = vec![
        token_asset(bridge_uusd_token_instance, Uint128::new(100)),
        token_asset(bridge_uluna_token_instance, Uint128::new(200)),
    ];
...
fn collect_maxdepth_test() {
    ...
    for t in vec![
        ...
        [
            token_asset(bridge2_token_instance.clone(), Uint128::from(100_000_u128)),
            token_asset(astro_token_instance, Uint128::from(100_000_u128)),
        ],
    ] {
        ...
        .execute_contract(
            ...
            &ExecuteMsg::UpdateBridges {
                add: Some(vec![
                    ...
                    (
                        token_asset_info(usdc_token_instance.clone()),
                        token_asset_info(test_token_instance),
                    ),
                    (
                        native_asset_info(uluna_asset.clone()),
...
fn collect_err_no_swap_pair() {
    ...
    for t in vec![(astro_token_instance, 10u128)] {
        let (token, amount) = t;
        mint_some_token(
            &mut router,
            owner.clone(),
    ...
    router
        .send_tokens(
            owner,
            maker_instance.clone(),
            &[coin(20, ukrt_asset), coin(30, uabc_asset)],
        )
        .unwrap();
    ...
    let e = router
        .execute_contract(maker_instance.clone(), maker_instance, &msg, &[])
        .unwrap_err();
...
fn collect_with_asset_limit() {
    ...
    check_balance(&mut router, staking, test_token_instance, Uint128::zero());
}
...
fn distribute_initially_accrued_fees() {
    ...
    router
        .send_tokens(
            owner.clone(),
            maker_instance.clone(),
            &[coin(110, uluna_asset), coin(100, uusd_asset)],
        )
        .unwrap();
