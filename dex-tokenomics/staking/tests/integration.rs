...
fn cw20receive_enter_and_leave() {
    ...
    mint_some_astro(&mut router, owner, astro_token_instance.clone(), ALICE);
...
fn should_not_allow_withdraw_more_than_what_you_have() {
    ...
    router
        .execute_contract(alice_address.clone(), astro_token_instance, &msg, &[])
        .unwrap();
    ...
    let res = router
        .execute_contract(alice_address, x_astro_token_instance, &msg, &[])
        .unwrap_err();
...
fn should_work_with_more_than_one_participant() {
    ...
    mint_some_astro(&mut router, owner, astro_token_instance.clone(), CAROL);
