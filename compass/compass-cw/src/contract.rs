...
pub fn execute(deps: DepsMut, env: Env, info: MessageInfo, msg: ExecuteMsg) -> Result<Response> {
    ...
    match payload {
        UpdateValset(new_valset, smart_contract_id) => {
            ensure!(smart_contract_id == id, "Wrong compass instance");
            update_valset(deps, env, info, &new_valset)
        }
        SubmitLogicCall(args, message_id, smart_contract_id, deadline) => {
            ensure!(smart_contract_id == id, "Wrong compass instance");
            submit_logic_call(deps, env, info, args, message_id, deadline)
        }
    }
}
